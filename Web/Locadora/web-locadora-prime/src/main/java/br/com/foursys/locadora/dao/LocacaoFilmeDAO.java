...
public class LocacaoFilmeDAO extends GenericDAO {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public ArrayList<LocacaoFilme> buscarFilmesPorLocacaoFilme(Locacao locacao) throws Exception {
		ArrayList<LocacaoFilme> retorno = new ArrayList<>();
		Session sessao = HibernateUtil.getSessionFactory().openSession();
		Criteria criteria = sessao.createCriteria(LocacaoFilme.class);
		criteria.add(Restrictions.eq("locacaoIdLocacao", locacao)); 
		criteria.addOrder(Order.asc("filmeIdFilme"));
		retorno = (ArrayList<LocacaoFilme>) criteria.list();
		sessao.close();
		return retorno;
	}
